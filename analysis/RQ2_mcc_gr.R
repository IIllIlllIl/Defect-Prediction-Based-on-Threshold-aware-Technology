...
for (a in unique(RQ2_unite$X)){
  ...
  if (length(base_mcc)>length(comp_mcc)){
    base_mcc<-base_mcc[1:length(comp_mcc)]
  }
  ...
  #mcc
  if(PValue_mcc<0.05&CliffDelta_mcc>=0){
    RQ2_unite$fill_p_mcc[RQ2_unite$X==a]="3"
  }
  else if(PValue_mcc<0.05&CliffDelta_mcc<0){
    RQ2_unite$fill_p_mcc[RQ2_unite$X==a]="1"
  }
  else{
    RQ2_unite$fill_p_mcc[RQ2_unite$X==a]="2"
  }
  
  
  if(abs(CliffDelta_mcc)<0.147){
    RQ2_unite$fill_c_mcc[RQ2_unite$X==a]="1"
  }
  else if(abs(CliffDelta_mcc)<0.33){
    RQ2_unite$fill_c_mcc[RQ2_unite$X==a]="2"
  }
  else if(abs(CliffDelta_mcc)<0.474){
    RQ2_unite$fill_c_mcc[RQ2_unite$X==a]="3"
  }
  else{
    RQ2_unite$fill_c_mcc[RQ2_unite$X==a]="4"
  }
}
...
bar_RQ2_mcc_rank2<-bar



#fig 5
bar5<-bar
p5<-ggplot(data=bar_RQ2_mcc_rank2, aes(x=X,y=value,fill=fill_c_mcc,colour=fill_p_mcc))+ylab("mcc")+
  geom_boxplot()+ 
...
ggsave(p5, file='RQ2_mcc_gr.pdf', width=8, height=4) # ????ָ????С??????Ϊ12cm????Ϊ10cm????Ҫָ??????·??
warnings()
