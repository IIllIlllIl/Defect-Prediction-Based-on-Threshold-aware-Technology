...
RQ2 <- ALL[(ALL$subsample=="ORIGIN"&ALL$ensemble=="ORIGIN"&(ALL$threshold=="AR"))|((ALL$subsample!="ORIGIN"|ALL$ensemble!="ORIGIN")&ALL$threshold=="DV"),]
RQ2<-RQ2[RQ2$repository!='SOFTLAB',]
RQ2<-RQ2[RQ2$classifier!='KNN',] 
RQ2_unite <-unite(RQ2,"X",subsample,ensemble,threshold)

RQ2_unite$X[RQ2_unite$X=='ORIGIN_ORIGIN_AR'] = 'AR'
...
for (a in unique(RQ2_unite$X)){
  baseline<-RQ2_unite[RQ2_unite$X=="AR",]
...
bar$X = factor(bar$X, levels=c("AR","US","OS","UOS","Smote","Bag","Bst","UBag","OBag","UOBag","SBag","UBst","OBst","UOBst","SBst"))
...
ggsave(p5, file='RQ2_recall_ar.pdf', width=8, height=4) # ????ָ????С??????Ϊ12cm????Ϊ10cm????Ҫָ??????·??
warnings()
