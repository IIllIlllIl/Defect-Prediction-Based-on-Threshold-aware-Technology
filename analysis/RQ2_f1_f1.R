...
RQ2 <- ALL[(ALL$subsample=="ORIGIN"&ALL$ensemble=="ORIGIN"&(ALL$threshold=="↑F1"))|((ALL$subsample!="ORIGIN"|ALL$ensemble!="ORIGIN")&ALL$threshold=="DV"),]
RQ2<-RQ2[RQ2$repository!='SOFTLAB',]
RQ2<-RQ2[RQ2$classifier!='KNN',] 
RQ2_unite <-unite(RQ2,"X",subsample,ensemble,threshold)

RQ2_unite$X[RQ2_unite$X=='ORIGIN_ORIGIN_↑F1'] = '↑F1'
...
for (a in unique(RQ2_unite$X)){
  baseline<-RQ2_unite[RQ2_unite$X=="↑F1",]
  base_f1<-baseline$f1
  comp_f1<-RQ2_unite$f1[RQ2_unite$X==a]
  
  if (length(base_f1)>length(comp_f1)){
    base_f1<-base_f1[1:length(comp_f1)]
  }
  ...
  #f1
  if(PValue_f1<0.05&CliffDelta_f1>=0){
    RQ2_unite$fill_p_f1[RQ2_unite$X==a]="3"
  }
  else if(PValue_f1<0.05&CliffDelta_f1<0){
    RQ2_unite$fill_p_f1[RQ2_unite$X==a]="1"
  }
  else{
    RQ2_unite$fill_p_f1[RQ2_unite$X==a]="2"
  }
  
  
  if(abs(CliffDelta_f1)<0.147){
    RQ2_unite$fill_c_f1[RQ2_unite$X==a]="1"
  }
  else if(abs(CliffDelta_f1)<0.33){
    RQ2_unite$fill_c_f1[RQ2_unite$X==a]="2"
  }
  else if(abs(CliffDelta_f1)<0.474){
    RQ2_unite$fill_c_f1[RQ2_unite$X==a]="3"
  }
  else{
    RQ2_unite$fill_c_f1[RQ2_unite$X==a]="4"
  }
}
...
bar$X = factor(bar$X, levels=c("↑F1","US","OS","UOS","Smote","Bag","Bst","UBag","OBag","UOBag","SBag","UBst","OBst","UOBst","SBst"))

bar_RQ2_f1_rank2<-bar



#fig 5
bar5<-bar
p5<-ggplot(data=bar_RQ2_f1_rank2, aes(x=X,y=value,fill=fill_c_f1,colour=fill_p_f1))+ylab("f1")+
  geom_boxplot()+ 
...
ggsave(p5, file='RQ2_f1_f1.pdf', width=8, height=4) # ↑↑ָ↑↑С↑↑↑Ϊ12cm↑↑Ϊ10cm↑↑Ҫָ↑↑↑·↑
warnings()
