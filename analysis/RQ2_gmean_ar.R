...
for (a in unique(RQ2_unite$X)){
  ...
  if (length(base_gmean)>length(comp_gmean)){
    base_gmean<-base_gmean[1:length(comp_gmean)]
  }
  ...
  #gmean
  if(PValue_gmean<0.05&CliffDelta_gmean>=0){
    RQ2_unite$fill_p_gmean[RQ2_unite$X==a]="3"
  }
  else if(PValue_gmean<0.05&CliffDelta_gmean<0){
    RQ2_unite$fill_p_gmean[RQ2_unite$X==a]="1"
  }
  else{
    RQ2_unite$fill_p_gmean[RQ2_unite$X==a]="2"
  }
  
  
  if(abs(CliffDelta_gmean)<0.147){
    RQ2_unite$fill_c_gmean[RQ2_unite$X==a]="1"
  }
  else if(abs(CliffDelta_gmean)<0.33){
    RQ2_unite$fill_c_gmean[RQ2_unite$X==a]="2"
  }
  else if(abs(CliffDelta_gmean)<0.474){
    RQ2_unite$fill_c_gmean[RQ2_unite$X==a]="3"
  }
  else{
    RQ2_unite$fill_c_gmean[RQ2_unite$X==a]="4"
  }
}
...
bar_RQ2_gmean_rank2<-bar



#fig 5
bar5<-bar
p5<-ggplot(data=bar_RQ2_gmean_rank2, aes(x=X,y=value,fill=fill_c_gmean,colour=fill_p_gmean))+ylab("gmean")+
  geom_boxplot()+ 
...
ggsave(p5, file='RQ2_gmean_ar.pdf', width=8, height=4) # ????ָ????С??????Ϊ12cm????Ϊ10cm????Ҫָ??????·??
warnings()
