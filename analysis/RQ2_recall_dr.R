...
RQ2 <- ALL[(ALL$subsample=="ORIGIN"&ALL$ensemble=="ORIGIN"&(ALL$threshold=="DR"))|((ALL$subsample!="ORIGIN"|ALL$ensemble!="ORIGIN")&ALL$threshold=="DV"),]
RQ2<-RQ2[RQ2$repository!='SOFTLAB',]
RQ2<-RQ2[RQ2$classifier!='KNN',] 
RQ2_unite <-unite(RQ2,"X",subsample,ensemble,threshold)

RQ2_unite$X[RQ2_unite$X=='ORIGIN_ORIGIN_DR'] = 'DR'
...
for (a in unique(RQ2_unite$X)){
  baseline<-RQ2_unite[RQ2_unite$X=="DR",]
  base_recall<-baseline$recall
  comp_recall<-RQ2_unite$recall[RQ2_unite$X==a]
  
  if (length(base_recall)>length(comp_recall)){
    base_recall<-base_recall[1:length(comp_recall)]
  }
  ...
  #recall
  if(PValue_recall<0.05&CliffDelta_recall>=0){
    RQ2_unite$fill_p_recall[RQ2_unite$X==a]="3"
  }
  else if(PValue_recall<0.05&CliffDelta_recall<0){
    RQ2_unite$fill_p_recall[RQ2_unite$X==a]="1"
  }
  else{
    RQ2_unite$fill_p_recall[RQ2_unite$X==a]="2"
  }
  
  
  if(abs(CliffDelta_recall)<0.147){
    RQ2_unite$fill_c_recall[RQ2_unite$X==a]="1"
  }
  else if(abs(CliffDelta_recall)<0.33){
    RQ2_unite$fill_c_recall[RQ2_unite$X==a]="2"
  }
  else if(abs(CliffDelta_recall)<0.474){
    RQ2_unite$fill_c_recall[RQ2_unite$X==a]="3"
  }
  else{
    RQ2_unite$fill_c_recall[RQ2_unite$X==a]="4"
  }
}
...
bar$X = factor(bar$X, levels=c("DR","US","OS","UOS","Smote","Bag","Bst","UBag","OBag","UOBag","SBag","UBst","OBst","UOBst","SBst"))

bar_RQ2_recall_rank2<-bar



#fig 5
bar5<-bar
p5<-ggplot(data=bar_RQ2_recall_rank2, aes(x=X,y=value,fill=fill_c_recall,colour=fill_p_recall))+ylab("recall")+
  geom_boxplot()+ 
...
ggsave(p5, file='RQ2_recall_dr.pdf', width=8, height=4) # ????ָ????С??????Ϊ12cm????Ϊ10cm????Ҫָ??????·??
warnings()
